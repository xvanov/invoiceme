 ...
 * 
 * Measures when the page structure is rendered using browser Performance Timing API.
 * This aligns with AC #6: "when the page loads, then the page renders in under 500ms"
 * 
 * Uses DOMContentLoaded event timing, which measures when the HTML is parsed and DOM is ready,
 * not when all API calls complete. This is the correct metric for "page renders".
 * 
...
export async function measurePageLoadTime(
  ...
  // Use browser's Performance Timing API to get actual DOMContentLoaded time
  const duration = await page.evaluate(() => {
    const perf = performance.getEntriesByType('navigation')[0] as PerformanceNavigationTiming;
    if (perf && perf.domContentLoadedEventEnd && perf.domContentLoadedEventStart) {
 ...
 * 
 * Measures when the new page structure is rendered after navigation using browser Performance Timing API.
 * This aligns with AC #8: "when navigation occurs, then the page transition completes in under 300ms"
 * 
 * Uses DOMContentLoaded event timing to measure when the new page is rendered.
 * 
 * @param page Playwright page instance
...
export async function measureNavigationTime(
  ...
  // Get the actual DOMContentLoaded timing from the browser
  const duration = await page.evaluate(() => {
    const perf = performance.getEntriesByType('navigation')[0] as PerformanceNavigationTiming;
    if (perf && perf.domContentLoadedEventEnd && perf.domContentLoadedEventStart) {
      return perf.domContentLoadedEventEnd - perf.domContentLoadedEventStart;
